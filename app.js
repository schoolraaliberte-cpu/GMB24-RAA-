...
const state = { user: null, profile: null, settings: null, sales: [], admin: false };
...
function calc(){
  ...
  return { todayCount: today.length, monthCount: sales.length, revenue, expenses, profit, tax };
}

function renderSales(){
  refs.salesList.innerHTML = state.sales.slice(0,20).map(s =>
...
refs.loginBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    refs.loginMsg.textContent = e.message;
  }
});
